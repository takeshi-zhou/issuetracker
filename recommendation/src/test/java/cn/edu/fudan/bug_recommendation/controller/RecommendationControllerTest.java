...
import cn.edu.fudan.bug_recommendation.BugRecommendationApplicationTests;
...
import cn.edu.fudan.bug_recommendation.service.impl.RecommendationServiceImpl;
import cn.edu.fudan.bug_recommendation.tool.TestDataMaker;
...
@WebAppConfiguration
public class RecommendationControllerTest extends BugRecommendationApplicationTests{
    ...
    public void addBugRecommendationTestSuccess() throws Exception{
        ...
        doNothing().when(recommendationService).addBugRecommendation(any(Recommendation.class));
    ...
    public void addBugRecommendationTestFail() throws Exception {
        ObjectWriter ow = mapper.writer().withDefaultPrettyPrinter();
        java.lang.String requestJson = ow.writeValueAsString(recommendation);
        PowerMockito.when(recommendationService, "addBugRecommendation", any(Recommendation.class)).thenThrow(new RuntimeException());
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post(
                "/bugRecommendation/add-bug-recommendation")
                .contentType(MediaType.APPLICATION_JSON_UTF8)
                .content(requestJson)
