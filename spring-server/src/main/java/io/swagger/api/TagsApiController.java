...
public class TagsApiController implements TagsApi {
    ...
    public ResponseEntity<ResponseBean> addTag(@ApiParam(value = " " ,required=true )  @Valid @org.springframework.web.bind.annotation.RequestBody RequestBody1 requestBody) {
    ...
    public ResponseEntity<ResponseBean> modifyTag(@ApiParam(value = " " ,required=true )  @Valid @org.springframework.web.bind.annotation.RequestBody RequestBody requestBody) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<ResponseBean>(objectMapper.readValue("{  \"msg\" : \"msg\",  \"code\" : 0,  \"data\" : \"{}\"}", ResponseBean.class), HttpStatus.NOT_IMPLEMENTED);
